...
-- 1. Muestra el producto del tipo "proteinas" más vendido.
...
-- 4. Muestra cual es el monitor que lleva más tiempo contratado.
select m.* from monitor m where m.fecha_contratacion like (select min(m2.fecha_contratacion) from monitor m2 );


-- 5. Muestra la rutina más usada por los usuarios y los datos del monitor que la realizó.
